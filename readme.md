...
Since the point of PoTO is to minimize PoW cost for a target security level (ie attack-cost), the likeliest type of short-range longer-chain attack on a PoTO blockchain is one where the attacker obtains many times the honest hashpower of the system to minimize the amount of stake they would need to control the chain. However, in such an attack, the number of PoW blocks mined per minted PoS block would drastically increase. There are two ways an attacker can longer-chain-attack the system. The easier way would be to slowly build up hashpower while honestly mining until you have a high percentage of the hashpower (eg 90% or 9 times the honest hashpower). In this case, when the attacker mines a secret chain, the honest chain (with the majority of active minters) will have a lot more PoS blocks than PoW blocks and the attacker chain will have a lot more PoW blocks than PoS blocks (unless they also have an enormous amount of stake, an amount which wouldn't be necessary for the attack). In the less likely case, the attacker suddenly unleashes never-before-seen hashpower to mine a secret chain (eg 9 times the honest hashpower). Here, the attacker chain also sees a lot more PoW blocks than PoS blocks.
...
Since PoA has stronger stake requirements than basic PoTO, combining PoA-style block signers would improve PoTO to a similar level of security. It could be done as follows:

All PoW and PoS blocks must be signed by a PoA-style N-stakeholder group determined by the hash value for PoW blocks and determined by the last address hash for PoS blocks. 

Its possible that multiple required minters might do the same thing as adding this PoA Extension (tho I think if we required N minters to mint a block, the security would be based around the probability that N out of 2N-1 active minters in the minter progression is the attacker). Its also possible that simply changing the chain-length equation to exponentiate the accumulated stake difficulty could have the same effect. More investigation is necessary.
...
A "25% attack" or [economic attack](https://bitcoinmagazine.com/articles/selfish-mining-a-25-attack-against-the-bitcoin-network-1383578440/) is where a selfish mining (and/or minting) strategy can allow a particular entity to gain more than their fair share of blocks and thereby either run honest miners out of the system by reducing their revenue below protitable levels or incentivize miners to join their coalition of selfish mining. Both of these outcomes increase the risk of a single entity or coalition gaining enough hashpower/stake to control the chain and do things like double-spend. [A paper](https://arxiv.org/abs/1311.0243) was written that talked about how Bitcoin is susceptible to this attack no matter how much hashpower the attacker has and suggested a partial-fix that when a miner has two potential chains of equal length to mine on top of, they randomly choose the chain to mine on top of. The paper says this makes it so the attacker requires 25% hashpower and goes on to say that theoretically there is no fix that could make this requirement larger than 33%. PoTO likely has this same problem, tho rather than being 25% it would be half the usual requirement of combined hashpower and stake, reducing the cost of attack to `(HashCost*ActiveStake)^.5` - however this is just conjecture until further analysis is done.

### PoW Monopolization Attack

For hybrid systems that rely on PoW and PoS, like PoTO and PoA, an attacker with greater than 50% of the hashpower can push out other miners and monopolize the generation of PoW blocks. For PoTO, the attacker would gain more than 50% of the hashpower, then simply refuse to mine on top of any chain that contains new PoW blocks created by another miner and instead selfishly mine on the chain where the last PoW block was their's. Since the blocks would be valid blocks propagated normally through the network, any honest minter would mint blocks on top of the attacker's blocks, giving the attacker's chain just as much PoS as the honest chain. However, it would have more PoW and therefore would be the longest chain. At that point, no other miner would be able to make money and would be forced to exit the network, giving the attacker 100% or almost 100% of the hashpower. The attacker could then use their near complete control of the hashpower to perform other attacks with very little coin ownership.

PoTO could fix this by introducing a penalty for signing on a PoW chain that doesn't end up winning. Require that any valid satoshi that comes up be part of an address with at least X coins (where X might be twice the median transaction fee rate times 200 bytes), and if that address signed on a PoW chain that doesn't end up winning then that X amount of coins will be deducted from their address and sent to the miner or minter who included proof of their signature. Minting on top of other PoS blocks with the same PoW block root can't be punished. This would hopefully incentivize rational minters to ignore shorter chains they have the opportunity to mint on, and only mint on top of the longest chain they're aware of (to minimize their chance of being punished). However, the punishment is small and hopefully shouldn't disincentivize honest minters (since there would be some collaterol damage - honest minters being punished simply for getting the wrong block first in a close race). Providing evidence of a minter signing a different chain should be limited to something like 5 blocks, so that after 5 blocks coin receivers can be sure the amount they received won't be reverted by a punishment. Since 5 blocks at 2 minutes would be about 10 minutes, this would provide a similar confidence of finalization as one bitcoin confirmation. 

The punishment amount should be proportional to the minter reward and further analysis is required to determine what proportion it should be. 
...
PoA is also suceptible to the PoW Monopolization Attack, where an attacker gains more than 50% of the hashpower and monopolizes the blocks, pushing any other miner out of business. As long as they're producing valid blocks and not mining in secret, they can likely obtain almost 100% of the hashpower this way and then perform any attack they want with a very small percentage of coins. The possibility of attack realistically reduces the practical security of PoA to about the level of a pure PoW system (perhaps a bit higher since the attack requires waiting for other miners to stop mining).
