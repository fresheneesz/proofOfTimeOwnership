...
- * [Rejected Proposals](#rejected-proposals)
  	  - [PoS-Absence Multiplier Extension](#pos-absence-multiplier-extension)
...
PoA seems to have exceptionally good security against double-spending attacks, better than basic PoTO (when N > 1), tho they didn’t give a formula for the minimum cost of attack and I haven’t derived it. (See [PoA-min-attack-cost.xlsx](https://github.com/fresheneesz/proofOfTimeOwnership/raw/master/PoA-min-attack-cost.xlsx) and [PoA-attack-cost.xlsx](https://github.com/fresheneesz/proofOfTimeOwnership/raw/master/PoA-attack-cost.xlsx) for calculations.)
...
### PoS-Absence Multiplier Extension

This proposal sounded like a good idea until it was realized that an attack would be possible (and actually easier) if the attacker was honestly mining with an enormous amount of hashpower (eg 99%) before beginning their attack. In this case, there would be no sudden decrease of PoS blocks per PoW block and so no way to do anything special about it. Here is the idea and analysis (now invalid because an easier attack method is known):
